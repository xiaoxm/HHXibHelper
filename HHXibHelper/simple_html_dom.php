...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        ...
        foreach ($matches as $m) {
            ...
            if ($this->dom->lowercase) {$tag=($tag); $key=($key);}
    ...
    function get_display_size()
    {
        ...
        if (isset($this->attr['style']))
        {
            ...
            if (isset($attributes['width']) && $width == -1)
            {
                ...
                if ((substr($attributes['width'], -2)) == 'px')
                {
            ...
            {
                // check that the last two characters are px (pixels)
                if ((substr($attributes['height'], -2)) == 'px')
                {
...
class simple_html_dom
{
    ...
    protected function read_tag()
    {
        ...
        if ($this->char==='/')
        {
            ...
            if ($parent_lower!==$tag_lower)
            {
                if (isset($this->optional_closing_tags[$parent_lower]) && isset($this->block_tags[$tag_lower]))
                {
                    ...
                    while (($this->parent->parent) && ($this->parent->tag)!==$tag_lower)
                        $this->parent = $this->parent->parent;

                    if (($this->parent->tag)!==$tag_lower) {
                ...
                else if (($this->parent->parent) && ($this->parent->parent->tag)===$tag_lower)
                {
                    $this->parent->_[HDOM_INFO_END] = 0;
                    $this->parent = $this->parent->parent;
                }
        ...
        {
            while (isset($this->optional_closing_tags[$tag_lower][($this->parent->tag)]))
            {
                $this->parent->_[HDOM_INFO_END] = 0;
                $this->parent = $this->parent->parent;
            }
        ...
        {
            // reset parent
            if (!isset($this->self_closing_tags[($node->tag)])) $this->parent = $node;
        }
        $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
